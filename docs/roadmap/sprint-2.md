...
### 7. 테스트 (1시간) — F3 으로 넘어갔습니다

F2 를 끝낼 때 러너가 없어서 못 썼습니다. 아래 둘은 순수 함수라 그대로 유효합니다 —
[F3 §6](sprint-3.md#6-테스트-1시간) 에서 vitest 를 켤 때 같이 씁니다.

- [ ] `visibleItems` 유닛 테스트
- [ ] `createPage` 하면 트리에 나타난다 / 없는 id로 `getPage` 하면 `not_found` / 저장 데이터가 깨져 있으면 안전하게 초기화된다

~~"만들기 → 목록에 나타남 → 클릭 → 열림" 통합 테스트 1개~~ — **뺍니다.** 렌더링이 필요해서
유닛으로 쓸 수 없습니다. F4 의 E2E 로 넘깁니다 ([테스트 정책](architecture.md#테스트)).
