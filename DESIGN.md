...
SEED 의 `t` 스케일은 11 · 12 · 13 · 14 · 16 · 18 · 20 · 22 · 24px 입니다(`t1`~`t9`). 제목1 30px 은 이 스케일 밖이라 `--knoc-` 로 따로 정했습니다. 제목3 의 20px 은 `t7` 과 같지만 셋이 한 스케일이라 같이 둡니다 — 하나만 다른 곳에서 오면 고칠 때 하나를 빠뜨립니다. 문서 제목 34px 도 스케일 밖이라 `--knoc-` 가 필요합니다 — **아직 만들지 않았습니다.** `PageTitle` 이 생기는 F3 §3 에서 추가합니다. 에디터 밖이라 `blocknote-bridge.css` 를 안 거칩니다.
