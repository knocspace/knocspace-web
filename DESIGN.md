...
| 드래그 핸들 메뉴 | 100px — 항목이 짧아 늘 바닥값 |

**두 가지가 어긋납니다.** 같은 자리에서 열리는 두 판이 3배 넘게 차이 나고, 슬래시 메뉴는 글자를
칠 때마다 목록이 걸러지면서 판이 같이 줄어듭니다.

그래서 **폭을 `--knoc-editor-menu-width` 로 한 번 정하고 둘 다 이 값을 읽습니다.** 340px 은 전체
목록의 336.4px 을 올림한 값입니다 — 지금 안 접히는 줄이 새로 접히지 않는 가장 작은 값이고,
BlockNote 의 천장 350px 아래라 그쪽 규칙과 싸우지도 않습니다.

```css
.bn-mantine.bn-root .bn-suggestion-menu  { width:     var(--knoc-editor-menu-width) }
.bn-mantine.bn-root .bn-drag-handle-menu { min-width: var(--knoc-editor-menu-width) }
```
...
`width` 와 안 싸우고, 쓰이는 폭이 `max(max-content, 340px)` 이 되어 결과가 정확히 340px 입니다.

**클래스는 `.bn-drag-handle-menu` 입니다.** 같은 판에 `.bn-menu-dropdown` 도 붙어 있지만 그건 포맷
툴바의 블록 타입 메뉴 · 코드 블록 언어 목록 · 색깔 서브메뉴가 같이 쓰는 이름이라, 그쪽까지 340px
로 늘어납니다.

**뜨는 자리는 그대로 BlockNote 것입니다.** 폭만 정하고 배치는 안 건드립니다. 넓은 창에서는 드래그
핸들 메뉴가 왼쪽으로 열려 사이드바 위를 덮는데, 좁아지면 mantine 이 알아서 오른쪽으로 뒤집습니다
(1024px · 820px 에서 확인). 잘리지 않으니 그대로 둡니다.
