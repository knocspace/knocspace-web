...
| `Toast` | `Snackbar` | 감쌉니다. 표면은 맞고 **위치 규칙만** 우리가 정합니다 |
...
- SEED `Snackbar` 그대로 — `min-height` 44px · `r2` 8px · 배경 `bg-neutral-inverted`
- **위치는 하단 왼쪽.** 아래·왼쪽에서 각 16px
- **기준은 뷰포트입니다.** 사이드바 폭을 따라가지 않습니다 — 사이드바 위에 겹쳐 뜹니다
- **폭은 280px 고정.** 글자수를 따라 움직이지 않습니다. 긴 문구는 줄을 늘립니다
...
하단 중앙이 아닌 이유: 트리에서 지운 것을 되돌리는 버튼이 트리 근처에 있어야 합니다. 넓은 화면에서 중앙은 방금 손이 있던 자리에서 800px 떨어집니다.
...
폭을 고정하는 이유: **되돌리기 버튼이 매번 같은 자리에 있어야 합니다.** SEED recipe 는 `width: 100%` 라, 하단 왼쪽으로 옮기면서 region 을 `w-auto` 로 만든 순간 폭이 내용을 따라갑니다. 재 보면 `저장했어요` 84px, 49자 문구 510px — 같은 삭제를 연달아 해도 버튼이 200px 넘게 이동합니다. 연타할 수 있는 버튼이 연타할 수 없는 자리에 있는 셈입니다.

280px 은 t4 14px 로 한글 15자 안팎이 한 줄에 들어가는 폭입니다. 우리 문구(`페이지를 삭제했어요`)는 대부분 한 줄, 사유가 붙으면 두 줄입니다.

사이드바를 따라가지 않는 것도 같은 이유입니다. 사이드바는 200~480px 로 **사용자가 끕니다.** 오른쪽에 붙이면 끌 때마다 토스트가 따라 움직여서, 폭을 고정해 얻은 것을 도로 잃습니다. 트리 맨 아래 두어 줄을 3~5초 가리는 건 감수합니다 — 되돌리기는 트리 근처에 있어야 하고, 가리는 쪽도 그 트리입니다.
...
| 폭 | `SnackbarRoot` 에 `w-toast`(`--knoc-toast-width`). 유틸리티 레이어가 `seed-components` 보다 뒤라 `!important` 가 필요 없습니다 |
| 버튼 자리 | **`Snackbar.Content` 를 반드시 씌웁니다.** 폭만 고정하고 이걸 빼면 `root` 가 그냥 flex 라 버튼이 메시지 꽁무니를 따라다닙니다 — 폭을 고정한 이유가 사라집니다. 최상위 export 가 없어 네임스페이스로 집습니다 |
| 뷰포트 기준 | SEED region 의 `left: 0` 을 그대로 둡니다. 우리가 덮은 것은 `right`·정렬·padding 뿐입니다 |
...
노션 웹앱 번들의 `Toaster` 청크를 읽고 맞춰 봤습니다.

| | Notion | KnocSpace |
|---|---|---|
| 위치 | `position: fixed · bottom: 0 · insetInlineStart`, 각 12px | 같음. **16px** |
...
| 폭 | 280 고정 | **같음** |
| 가로 기준 | `insetInlineStart` 가 앱 환경에서 계산됩니다 — **어떤 값인지 못 밝혔습니다.** 여백 `calc(12px + var(--safe-padding-left))` 의 그 변수는 사이드바가 아니라 `env(safe-area-inset-left)` 였습니다 | 뷰포트 왼쪽 |
| 표면 | `min-height` 40 · `r` 8 · 앱 테마와 무관하게 항상 다크 | `min-height` 44 · `r2` 8 · `bg-neutral-inverted` |
