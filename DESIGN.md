...
| 에디터 떠 있는 메뉴 폭 | 340px (슬래시 · 드래그 핸들 공통, §7) | `--knoc-editor-menu-width` |
...
**예외는 에디터의 선택 오버레이 둘뿐입니다** — `--knoc-color-selection-surface`(라이트 8% · 다크 16%)와 `--knoc-color-selection-ring`(30%). 단계 한 벌이 아니라 자리가 정해진 semantic 값 둘이고, `bg-brand-weak` 로는 **물리적으로 못 하는** 자리입니다. 그 색이 깔리는 곳이 `:after { inset: 0 }` 으로 블록 위에 덮는 오버레이라, 불투명하면 글자가 가려집니다. 자세한 것은 §7.
...
**그런데 변수가 안 달린 곳이 있습니다.** F3 §0 에서 전수 조사했습니다. 아래 「예외 열여섯 줄」과
...
「삽입선」 셋만 자손 선택자가 아니라 클래스 한 겹입니다 — 그 div 가 에디터 밖에 그려지기 때문이고,
해당 절에서 따로 설명합니다.
...
| **삽입선 — 블록 드롭** | 3 | 옵션으로 못 주는 것만 얹기 — **유일하게 에디터 밖** |
| **블록 색이 덮는 자리** | 3 | 색이 아니라 **상자**를 다시 잡기 — 위의 블록 면과 한 벌 |
| **글자색 — 슬래시 메뉴 그룹 라벨** | 1 | 뒤 규칙에 눌린 앞 규칙의 의도 되살리기 |
| **폭 — 슬래시 메뉴 · 드래그 핸들 메뉴** | 2 | **없던 선언을 새로 얹기** — 유일한 치수 |
| **크기·간격 — 사이드 메뉴 손잡이** | 2 | **박힌 값 덮기 + 없던 선언 얹기** — 치수 |
...
(**끌 때의 선택 사각형도 여기 안 셉니다.** 그 `div` 역시 에디터 밖이고 클래스 한 겹이라
삽입선과 같은 쪽입니다 — 「선택」 절 끝에 붙여 뒀습니다.)
...
`classList.remove` 가 우리 데코레이션까지 걷어 갑니다. `Esc` 뒤에 `shift`+`↓`, 여백에서 아래로
끌기(첫 줄이 잠깐 혼자 잡힙니다) 둘 다 그 길을 지납니다. 이름을 갈라 두면 서로 안 건드리고,
칠하는 값은 위 규칙이 `:is()` 로 둘을 같이 짚어 **한 자도 다르지 않습니다.**
...
그것만 감춥니다. 블록 하나일 때는 ProseMirror 가 `.ProseMirror-hideselection` 으로 이미 하는
일이고, 같은 방법을 같은 값으로 따라 쓴 것입니다.
...
— 그 알파를 깔면 나오는 색이 트리 행의 `bg-brand-weak` 와 거의 같습니다. 토큰을 두 벌 두지
않고도 같은 모양이 됩니다.

**그 알파가 모드마다 다릅니다 — 라이트 8% · 다크 16%.** 처음에는 한 벌로 뒀습니다. `bg-brand-solid`
가 이미 모드별로 갈리니(purple-700 ↔ 600) 색은 따라오고 세기는 같아도 된다고 본 것인데, **재
보니 틀렸습니다.** 갈리는 것은 **색**이고, `weak` 단계가 다크에서 훨씬 진한 것은 **세기**라 따로
놉니다.

| | 트리 행 (`bg-brand-weak`) | 문서 바탕 | 그 색을 내는 알파 |
|---|---|---|---|
| 라이트 | `#f5f3fe` | `#ffffff` | **8%** |
| 다크 | `#28213b` | `#16171b` | **16%** |

8% 로 두면 다크에서 합성색이 `rgb(32,30,44)` 인데 트리 행은 `rgb(40,33,59)` 입니다 — 절반쯤에서
멈춥니다. 실제로 다크에서 고른 블록이 거의 안 보였습니다. **링(30%)은 안 건드립니다** — 저건
면이 아니라 선이라 같은 알파에서도 픽셀당 세기가 다르고, 다크에서 `rgb(58,48,90)` 으로 이미
충분히 섭니다. 다크 한 줄은 `knocspace.css` 의 `:root` **뒤에** 옵니다(명시도가 같으면 나중 것이
이깁니다).

**덮는 자리는 글줄 상자입니다 — 좌우로는 넓히고 위아래로는 좁힙니다.**

좌우는 밖으로 6px 나갑니다. **글자가 상자 벽에 붙으면 답답하기 때문**입니다. 글자를 오른쪽으로
밀 수는 없습니다 — 본문 왼쪽 끝은 문서 제목과 맞춰 둔 자리라 움직이면 둘이 어긋납니다. 그래서
글자는 두고 상자를 밖으로 내보냅니다.

**위아래는 반대로 상자 안쪽으로 `gap/2 − rim` 만큼 들어옵니다.** 블록 사이 여백을 **BlockNote
가 두는 자리에 그대로 두고 값만 키웠기** 때문에 그럴 자리가 생깁니다 — `.bn-block-content` 의
`padding: 3px 0` 을 6px 로 올려 블록 사이가 12px 이 됩니다.

**밖으로 옮기면 안 됩니다.** `.bn-block-outer` 의 margin 으로 빼 봤다가 두 가지가 깨졌습니다.
그 띠가 어느 블록에도 안 속해서 **마우스를 올려도 `⠿` 손잡이가 안 뜨고**(BlockNote 는
`event.target` 에서 `[data-node-type="blockContainer"]` 를 타고 올라가 찾습니다), 손잡이의
세로 자리를 재는 코드가 그 `padding` 을 읽는데 0 이 되어 제목에서 18px 어긋났습니다
(`lib/blocknote-side-menu.ts`). padding 이면 여백이 블록의 몸이라 마우스도 걸리고 잰 값도
삽니다.
...
| | 상자 |
| --- | --- |
| 블록 색 | **글줄 상자** (좌우로만 3px) — 위아래는 `padding` 을 `margin` 으로 옮겨, 자리는 그대로 두고 칠하는 면만 줄입니다 |
| 선택 면 | 글줄 상자 + 사방으로 — 위아래 `rim` 4px, 좌우 6px |
| 고른 줄 사이 | `block-gap − rim × 2 = 12 − 8 = 4px` |

**셋이 한 예산을 나눠 씁니다.** 선택 면의 높이는 `글줄 + rim × 2`, 고른 줄 사이 틈은
`block-gap − rim × 2` 라, 둘의 합이 늘 `글줄 + block-gap` 입니다. 바를 높이면 틈이 줄고,
둘 다 키우려면 `block-gap` — 곧 문서의 세로 리듬 — 을 늘려야 합니다.

BlockNote 기본은 6px 이었습니다. 그 값으로는 바가 `글줄 + 2px` 밖에 안 되어 블록 색과
구별이 안 갔습니다. 12px 은 바를 `글줄 + 8px` 로 두면서 틈 4px 을 남기는 값입니다.

**제목만 선택 면이 위로 더 나갑니다.** 제목의 위 여백이 18px 이라 그대로 두면 제목 앞에서만
틈이 10px 로 벌어져 한 덩어리로 안 읽힙니다. 면을 그 차이만큼 위로 밀어 틈을 같게 맞춥니다 —
움직이는 것은 칠하는 면뿐이고, 안 고른 상태의 여백은 그대로입니다.

한 줄만 고를 때도 같은 상자입니다. 한 줄일 때만 두툼하면 `Esc` 로 한 줄을 고른 뒤 `shift`+`↓`
를 누르는 순간 그 줄이 얇아집니다.

**제목의 18px 은 그 자리에 그대로 둡니다.** `padding-block` 을 덮으면서 같이 지워지므로 뒤에서
되돌립니다. 여백이 블록 안에 있으니 제목의 선택 면은 그만큼 위로 길어지고, 고른 줄 사이의
틈은 블록 종류와 무관하게 늘 같습니다 — 면이 상자 안으로 들어오는 거리가 하나뿐이기 때문입니다.
...
여기 못 씁니다 — §2 의 alpha 예외가 이것입니다. 다음 절의 블록 드롭 삽입선이 셋째인데,
그건 가려지지 않으려는 것이 아니라 **덜 무거우려는** 알파라 이유가 다릅니다.

**8% · 30% 는 BlockNote 값을 그대로 옮긴 것입니다.** `#64a0ff14`(8%) · `#64a0ff4d`(30%). 색만
바꾸고 세기는 남깁니다 — 인용의 `border-left-color` 와 같은 원칙입니다. **다크의 면 16% 하나만
예외입니다** — 저쪽 값은 라이트 기준이라 다크에서 절반쯤에서 멈춥니다(바로 위). `box-shadow` 는 색만
...
##### 끄는 동안의 선택 사각형 — 위의 여섯에 안 셉니다

여백에서 끌어 여러 블록을 고를 때 따라오는 사각형입니다 (Notion 규격, F3). **위의 여섯과 갈라
두는 것은 자리 때문입니다** — 이 `div` 는 에디터 밖에 그려지고 선택자가 클래스 한 겹이라,
다음 절의 삽입선과 같은 쪽입니다. 붙이는 자리도 삽입선과 같은 `view.dom.offsetParent`
(우리 화면에서는 `<body>`)이고, 만드는 것은 `pages/page-editor/lib/blocknote-marquee-selection.ts`
입니다.

```css
.knoc-marquee {
  position: absolute; pointer-events: none;
  border: 1px solid  var(--knoc-color-selection-ring);
  border-radius:     var(--seed-radius-r1);
  background-color:  var(--knoc-color-selection-surface);
}
```

**여기만 면에 테두리가 붙습니다.** 블록 선택은 면만 쓰고 링이 없는데(바로 위), 이 사각형은
**이미 8% 로 칠해진 블록 위에** 겹쳐 놓입니다 — 면만으로는 자기 가장자리가 안 보입니다. 값은
새로 만들지 않고 노드 링이 쓰는 `-ring`(30%) 을 그대로 씁니다. 알파 **단계**를 늘리지 않는다는
§2 가 그대로 지켜집니다.

**반경 `r1` 은 블록 선택 면과 같은 값입니다.** 사각형이 지나간 자리에 같은 모양의 면이 남는
것이라, 두 상자의 모서리가 어긋나면 손이 지나간 자리와 잡힌 자리가 다른 물건으로 보입니다.

**움직임은 안 줍니다.** 자리가 마우스에 1:1 로 매여 있어서 `transition` 을 걸면 사각형이 손끝을
뒤따라옵니다. 다음 절의 삽입선과 반대입니다 — 저쪽은 자리가 블록 단위로 튀어서 미끄러짐이
필요했습니다.

#### 삽입선 하나 — 블록 드롭

**앞 절의 「드롭 커서」와 다른 것입니다.** 저건 표 안에서 열·행을 옮길 때 뜨는
`.bn-table-drop-cursor` 이고, 이건 `⠿` 로 **블록**을 끌 때 「여기에 놓인다」를 가리키는
가로선입니다. F3 §0 조사에서 빠져 있었습니다 — BlockNote 기본은 5px 짜리 `#ddeeff` 입니다.
§1 이 「커서, 삽입선」을 강조색 하나로 못 박아 뒀고, 표 쪽의 같은 선은 이미 그 색입니다.

**다만 이 선만 솔리드가 아닙니다.** 앞 절의 「뒤의 둘은 솔리드입니다」는 표 안의 짧은 막대
얘기입니다. 이 선은 본문을 가로지르는 600px 짜리 가로선이라 같은 색이어도 눈에 닿는 면적이
두 자릿수로 차이 나서, 솔리드로 두면 문서 위에 보라색 띠가 하나 그어집니다.
`--knoc-color-drop-indicator` 는 `bg-brand-solid` 의 **60%** 입니다 — 색이 옅어지는 것과 선
아래가 비치는 것을 알파 하나로 같이 합니다(§2 의 alpha 예외 셋째). 흰색을 섞으면 다크에서
되레 밝아져서 안 됩니다. 링 30% 보다 진한 것은, 저건 이미 고른 것을 감싸는 테두리고 이건
아직 아무 일도 안 일어난 자리를 먼저 가리키는 선이기 때문입니다.

**색과 굵기는 CSS 가 아닙니다.** 확장이 만드는 div 라 자리·크기·색이 전부 인라인 `style` 로
박힙니다. 굵기는 좌표 계산에도 쓰여서(경계선 위아래로 `width/2` 씩) `!important` 없이는 CSS 로
못 이깁니다. 라이브러리가 옵션을 열어 뒀으니 거기서 줍니다.

```ts
// pages/page-editor/model/content-editor.ts — useCreateBlockNote 옵션
dropCursor: { width: 3, color: "var(--knoc-color-drop-indicator)" }
```

**3px 은 5px 을 그냥 줄인 값이 아닙니다.** 옅은 하늘색에서 우리 보라로 오면서 같은 굵기가 두 배로
무거워졌습니다. 블록 사이 여백이 3px(`--knoc-editor-block-pad-y`)이라 선이 그 틈을 정확히
채우고, Notion 도 이 자리에서 2~3px 입니다.

**옵션에 없는 둘만 CSS 로 얹습니다 — 둥근 끝과 움직임입니다.**

```css
/* .bn-container 로 시작하지 않습니다 — 아래 「에디터 밖」 */
.prosemirror-dropcursor-block,
.prosemirror-dropcursor-inline {
  border-radius: var(--seed-radius-full);
  animation: knoc-drop-indicator-in 100ms ease-out;
}

.prosemirror-dropcursor-block.prosemirror-dropcursor-block {
  transition-property: top, bottom, left, width;
}
```

**가로로도 미끄러지게.** BlockNote 는 `top`·`bottom` 만 전환에 걸어 둡니다. 선이 세로로는
미끄러지고 가로로는 순간이동한다는 뜻이라, 들여쓰기가 다른 블록 사이를 지날 때마다(목록 안팎)
튑니다. 같은 전환에 `left`·`width` 를 더하고 **duration 과 timing 은 BlockNote 값(150ms)을
남깁니다** — 인용의 `border-left-color` 와 같은 원칙입니다. 움직일 것만 더하고 세기는 둡니다.

**뜨는 순간의 페이드가 `transition` 이 아니라 `animation` 인 이유.** 이 div 는 드래그 한 번마다
새로 만들어지고 놓으면 지워집니다. 처음 그려지는 자리엔 전 값이 없어서 transition 이 안 걸립니다.
`@keyframes knoc-drop-indicator-in` 은 `opacity: 0` 한 줄뿐입니다. `prefers-reduced-motion: reduce`
에서는 이것도 BlockNote 의 150ms 도 같이 끕니다 — 우리가 더한 것만 끄면 정작 원래 있던
미끄러짐이 남습니다(§10 Skeleton 과 같은 규칙입니다).

**이 묶음만 에디터 밖입니다.** 확장이 이 div 를 `view.dom.offsetParent` 에 붙이는데, 우리
화면에서는 그게 `<body>` 입니다 — `.bn-container` 로 짚으면 안 잡힙니다. 클래스 이름은
prosemirror-dropcursor 것을 BlockNote 가 그대로 물려받은 것이라 `.ProseMirror-selectednode` 와
같은 계층입니다. 그래서 자손 선택자가 없고, 두 겹으로 짚는 곳은 BlockNote 가 유일하게 선언해 둔
`transition-property` 한 줄뿐입니다 — 에디터 CSS 가 지연 로드라 동점이면 집니다.

**실패해도 모양만 각져집니다.** 색과 굵기는 인라인이라 남습니다.
#### 블록 색 셋 — 색이 덮는 자리
...
`＋` 와 `⠿` 입니다. 폭 둘과 같은 성격이라 이어서 읽으면 됩니다 — 색도 글꼴도 아닌 **치수**입니다.

**크기.** BlockNote 는 24px 버튼 안에 22px 짜리 그림을 넣습니다.
...
양옆으로 1px 씩만 남습니다. `＋` 는 막대 둘, `⠿` 는 점 여섯이고 **둘 다 fill 로 그린 도형**이라,
그 크기 그대로 본문 옆에서 무겁게 보입니다.

**굵기만 줄일 수는 없습니다.** fill 이라 `stroke-width` 가 안 통하고, 그림 자체를 갈아 끼우는 것은
§6 「슬래시 메뉴 아이콘」과 같이 열려 있는 결정입니다. 그래서 크기로 무게를 줄입니다 — 18px 이면
`＋` 의 막대가 1.5px → 1.2px, `⠿` 의 점이 3.7px → 3.0px 입니다.

**간격.** 사이드 메뉴는 블록 왼쪽에 딱 붙어 섭니다(`placement: left-start`, 가로 오프셋 0).
판 오른쪽에 패딩을 주면 오른쪽 끝은 제자리에 두고 그림만 왼쪽으로 물러납니다.

**4px 인 것은 남는 자리가 6px 이기 때문입니다.** 거터가 54px(`.bn-editor { padding-inline: 54px }`)
이고 판은 24px 버튼 둘이라 48px 입니다. 6px 을 다 쓰면 판의 왼쪽 끝이 `.bn-editor` 밖으로 나갑니다
— 사이드 메뉴를 여닫는 것이 그 안에서의 마우스 위치라, 거터 안에 남겨 둡니다.

```css
.bn-mantine.bn-root .bn-side-menu             { padding-right: var(--knoc-editor-handle-gap) }
.bn-mantine.bn-root .bn-side-menu .bn-button svg {
  width:  var(--knoc-editor-handle-icon);
  height: var(--knoc-editor-handle-icon);
}
```

**`.bn-button` 을 하나 더 태우는 것은 명시도 때문입니다.** 저쪽이 (0,3,1) 이라 `.bn-mantine.bn-root`
두 겹만으로는 (0,3,1) 로 비깁니다. `.bn-side-menu` 와 `.bn-button` 을 더해 (0,4,1) 로 이깁니다.
`＋` 와 `⠿` 둘 다 이 클래스를 답니다.

**세로 자리는 여기서 안 건드립니다.** 손잡이를 블록 첫 줄 가운데에 맞추는 것은 CSS 가 아니라
floating-ui 미들웨어입니다 — `blocknote-side-menu.ts`, 제목 크기를 바꾸면 BlockNote 의 상수가
전부 틀어지기 때문에 그 자리에서 재서 계산합니다.
...
#### 폭 둘 — 슬래시 메뉴 · 드래그 핸들 메뉴

**둘은 같은 자리에서 열립니다.** 사이드 메뉴의 `＋` 를 누르면 슬래시 메뉴가, `⠿` 를 누르면 드래그
핸들 메뉴가 뜹니다. 그런데 BlockNote 기본값으로는 폭이 따로 놉니다 — 둘 다 내용을 따라가게 두고
바닥·천장만 다르게 주기 때문입니다.

```css
.bn-mantine .bn-suggestion-menu      { max-width: 350px }   /* 천장만 */
.bn-side-menu .mantine-Menu-dropdown { min-width: 100px }   /* 바닥만 */
```

재 본 값입니다 (1440×900, ko 사전).

| 무엇 | 폭 |
|---|---|
| 슬래시 메뉴 — 전체 목록 | 336.4px |
| 슬래시 메뉴 — `/제목` (6항목) | 267.5px |
| 슬래시 메뉴 — `/제목 1` (1항목) | 137.1px |
| 블록 메뉴(⠿) | 100px — 항목이 짧아 늘 바닥값 |

**두 가지가 어긋납니다.** 슬래시 메뉴는 글자를 칠 때마다 목록이 걸러지면서 판이 같이 줄어들고,
블록 메뉴는 내용이 뭐든 늘 바닥값입니다. 그래서 폭을 둘 다 정해 줍니다.

**값은 서로 다릅니다.** 한동안 둘이 같은 340px 을 읽었습니다 — 같은 자리에서 열리니 폭도 같아야
한다고 봤습니다. **하는 일이 다른 판이라 갈라 냈습니다.** 슬래시 메뉴는 이름과 설명 두 줄짜리
25항목을 세우는 목록이고, 블록 메뉴는 네 줄짜리 명령 판입니다. 340px 을 주면 `복제` 한 단어 옆에
빈자리가 250px 남습니다.

- **슬래시 메뉴 340px** (`--knoc-editor-menu-width`) — 전체 목록의 336.4px 을 올림한 값입니다.
  지금 안 접히는 줄이 새로 접히지 않는 가장 작은 값이고, BlockNote 의 천장 350px 아래라 그쪽
  규칙과 싸우지도 않습니다
- **블록 메뉴 200px** (`--knoc-block-menu-width`) — §10 Menu 의 기본 폭입니다. Notion 을 잰
  265px 에서 한 번 더 좁혔습니다. 줄이 `전환`·`색상`·`복제`·`삭제` 두 글자씩이라 265px 에서는
  이름 오른쪽이 계속 비어 있었고, **폭이 판이 뜨는 쪽까지 정하기 때문입니다** — 아래

```css
.bn-mantine.bn-root .bn-suggestion-menu  { width:     var(--knoc-editor-menu-width) }
.bn-mantine.bn-root .bn-drag-handle-menu { min-width: var(--knoc-block-menu-width) }
```

**서브메뉴(전환 · 색상)에는 폭을 안 겁니다.** 부모보다 좁은 것이 맞고(Notion 도 그렇습니다),
`.bn-drag-handle-menu` 는 부모 판에만 붙는 이름이라 자동으로 그렇게 됩니다.

**한쪽만 `min-width` 인 것이 이상해 보이는데, 그럴 수밖에 없습니다.** 드래그 핸들 메뉴는 mantine
Popover 가 그리는 판이라 `width: max-content` 가 **인라인 style 로** 박힙니다. 인라인은 명시도로
못 이기고, 이 파일은 `!important` 를 안 씁니다. `width` 로 적으면 조용히 아무 일도 안 하고 100px
그대로입니다 — 실제로 그렇게 썼다가 재 보고 고쳤습니다. `min-width` 는 다른 속성이라 인라인
`width` 와 안 싸우고, 쓰이는 폭이 `max(max-content, 265px)` 이 되어 결과가 정확히 265px 입니다.

**클래스는 `.bn-drag-handle-menu` 입니다.** 같은 판에 `.bn-menu-dropdown` 도 붙어 있지만 그건 포맷
툴바의 블록 타입 메뉴 · 코드 블록 언어 목록 · 전환 · 색상 서브메뉴가 같이 쓰는 이름이라, 그쪽까지
200px 로 늘어납니다.

**뜨는 자리는 그대로 BlockNote 것입니다 — 다만 폭이 그 자리를 정합니다.** BlockNote 는 블록
메뉴를 손잡이 **왼쪽**에 열려고 하고(`position="left"`, Notion 과 같은 쪽입니다), 왼쪽에 자리가
모자라면 mantine 의 `flip` 이 오른쪽으로 뒤집습니다.

**그 "자리" 를 재는 기준이 창이 아닙니다.** 문서가 굴러가는 칸(`AppLayout` 의 `overflow-y-auto`)이
floating-ui 의 clipping ancestor 라, 왼쪽 경계가 사이드바 오른쪽 끝입니다. 1440px 창 · 사이드바
240px 에서 손잡이 왼쪽에 남는 자리가 266px 이고, 265px 짜리 판은 여기에 딱 걸려 오른쪽으로
뒤집혔습니다. 200px 은 남습니다.

창이 아주 좁아지면(≈1280px 아래) 그때는 다시 오른쪽으로 뒤집힙니다. 판이 잘리는 것보다 낫고,
이건 BlockNote·mantine 의 안전장치라 그대로 둡니다. 늘 왼쪽이어야 한다면 floating UI 를
`document.body` 로 내보내 경계를 창으로 넓히는 길이 있는데(에디터의 `portalTarget`), 슬래시 메뉴 ·
포맷 툴바까지 같이 옮겨 가는 변경이라 따로 봅니다.
...
|---|---|
| 모든 색 — 예외 목록의 색 전부와 `--mantine-` 둘 포함 | 블록 상하 간격의 **자리** — 값만 키웁니다 |
...
| 선택·커서·삽입선 (§1 · 예외 여섯) | 한 블록 **안**에서의 커서·선택 — 글자·단어·줄, 표 셀 |
| `⠿` 를 눌렀을 때의 **블록 선택** (§7) | |
| **줄을 넘어가는 선택** — 끌기 · `shift`+클릭이 블록을 통째로 잡습니다 (§7) | |
| 블록 선택 중의 **키보드** — `Esc` `Enter` 화살표 `⌘A` `⌘D` `⌘X` `⌘V` (§7) | 나머지 단축키 전부 — `⌘Z` `Tab` `⌘`+`shift`+화살표 · 마크다운 입력 |
| 제목 ↔ 본문 **커서 넘김** — `Enter` `↑` `↓` `Backspace` | BlockNote 에는 페이지 제목이 없습니다 |
| 블록 전환 **숫자키** — Notion 번호(`⌘⌥0`~`8`)로 덮습니다 | 나머지 블록 단축키 |
| `⌘Enter` — 체크박스 · 토글 여닫기 | |
| 마크다운 `>` — 토글 목록 (인용은 `"`) | 나머지 마크다운 입력 전부 |
| 포맷 툴바가 **언제 뜨는지** — 블록 하나를 골랐을 때는 안 띄웁니다 (§7) | 포맷 툴바의 **표면**과 항목 (F3 §2 까지) |
| 코드 블록 언어 선택기 — 스펙의 `render` 만 (§7) | |
| 제목 크기 · 줄간 (§2) — `--level` 로 넘깁니다 | |
| 사이드 메뉴의 **세로 위치** — 첫 줄을 재서 맞춥니다 | |
| 슬래시 메뉴 · 드래그 핸들 메뉴의 **폭** (§7) — 뜨는 **자리**는 BlockNote 것입니다 | |
| | 파일·이미지 블록 계열 색 (F3 밖) |
...
제목이 하나도 없는 목차에도 빈 화면 컴포넌트를 쓰지 않습니다. 문서 안에 든 블록 하나가 아이콘과 제목을 세우면 그 자리만 화면처럼 읽힙니다. 빈 문서 첫 줄과 같은 규격입니다 — 한 줄, 버튼 없음.

**여기 한 줄에만 배경을 깝니다** (`bg-informative-weak`, 반경 `r1`, 안쪽 여백 8/4px). 빈 문서 첫 줄과 다른 점이고, 자리가 다르기 때문입니다. 빈 문서의 첫 줄은 커서가 이미 그 줄에 있어서 어디를 말하는지가 분명하지만, 빈 목차는 본문 한가운데 끼어 있어 글자만 두면 방금 넣은 블록이 본문 한 줄과 구별되지 않습니다. 띠가 곧 "여기가 목차 자리" 라는 표시고, 제목을 하나 넣는 순간 목록으로 바뀌면서 사라집니다. 상태색 중 `informative` 인 것은 §1 그대로입니다 — 경고도 오류도 아닌 안내입니다.

**글자는 본문 16px 이 아니라 14px 입니다** (`--knoc-text-toc-entry` → `t4`). 값의 출처는 Notion 이고, 블록 전체가 한 크기입니다 — 항목도 빈 목차 안내도 같습니다. 크기로 층을 가르지 않습니다. 제목1 항목을 크게 세우면 목차가 그 자체로 제목 덩어리가 되고, 층은 들여쓰기가 이미 말하고 있습니다.

**항목 글자를 `fg-neutral` 로 두지 않습니다.** 목차는 본문을 가리키는 표지판이지 본문이 아닙니다. 제목들과 같은 진하기면 문서 맨 위에서 제일 진한 덩어리가 됩니다. 더 옅은 `fg-neutral-subtle` 로는 안 갑니다 — 라이트에서 흰 바탕 대비가 3.2:1 이라 본문 크기 글자에 필요한 4.5:1 에 못 미칩니다(`muted` 는 7.5:1). 누르는 것이라 더 그렇습니다.

**들여쓰기가 트리 행(14px)보다 넓은 것**은 목차에는 층을 말해 주는 것이 자리뿐이기 때문입니다. 트리에는 접기 화살표와 아이콘이 층을 같이 말해 주지만, 목차 항목은 같은 크기의 글자가 줄줄이 설 뿐이라 한 칸이 16px 이면 "들여쓴 것" 이 아니라 "줄이 삐뚤어진 것" 으로 보입니다.

| | |
|---|---|
| 빈 목차 | `제목을 넣으면 여기에 모여요.` (`fg-neutral-muted`). 옅은 파랑 띠 위에 — 위 설명 |
| 항목 글자 | `t4-regular` (14px), `fg-neutral-muted`. hover 에서 `fg-neutral` 로 진해집니다 |
| 항목 밑줄 | **늘 그어 둡니다.** 색은 `stroke-neutral-muted` — 글자보다 옅습니다 |
| 층 들여쓰기 | 층마다 24px (`comfy-5` · `comfy-7`). 셋째 층이 끝입니다 |
| 층 세는 법 | 제목 크기가 아니라 **앞 제목과의 관계** 입니다. 제목1·3·2·3·1 이면 0·1·1·2·0 |
| 슬래시 메뉴 이름 | `목차` |
| 슬래시 메뉴 설명 | `페이지의 개요를 표시합니다.` |
| 아이콘 | `IconDocumentLine` — 테두리 안에 가로선 셋, 마지막 줄이 짧습니다 |

**설명이 이름에 없는 것을 얹습니다** (§8). "목차" 는 결과의 이름이고, 사용자가 모르는 것은 그게 페이지 전체를 훑는 개요라는 쪽입니다.

**슬래시 메뉴 설명만 합니다체입니다 — §8 해요체의 유일한 예외입니다.** 이 줄이 앉는 자리가 우리 화면이 아니라 BlockNote `ko` 사전이 쓴 메뉴 한가운데이기 때문입니다. 위아래가 `번호가 매겨진 목록을 추가합니다.` · `이미지 파일을 업로드합니다.` 인데 여기만 해요체면 그 한 줄이 남의 글처럼 읽힙니다. 사전 20항목을 우리 문구로 덮는 것이 아니라면 이쪽이 사전에 맞춥니다.

**예외는 이 한 줄뿐입니다.** 같은 블록이라도 빈 목차 문구는 문서 본문에 앉으므로 해요체입니다. F3 §2 에서 메뉴를 SEED 표면으로 갈아 끼우면서 20항목의 문구까지 우리가 쓰게 되면, 그때 이 줄도 해요체로 돌아옵니다.

**아이콘은 슬래시 메뉴에서 우리가 고른 유일한 것입니다.** 나머지 항목의 아이콘은 아직 BlockNote 기본(인라인 SVG)이고, 그 전체를 교체하는 결정은 §6 에 열려 있습니다. 목차만 먼저 고른 것은 BlockNote 에 없는 블록이라 기본 아이콘 자체가 없기 때문입니다 — seed-icon 안에서 닫혔으므로 §6 의 A·B·C 중 어느 것을 고르든 이 값은 안 바뀝니다.

---

### 블록 메뉴 (에디터) — F3 에서 확정

블록 손잡이(⠿)를 눌렀을 때 뜨는 메뉴입니다. **네 줄입니다 — 전환 · 색상 · 복제 · 삭제.**

BlockNote 기본은 삭제 · 색깔 · 표 머리글 셋입니다. 표 머리글은 뺐습니다. 표 안에서만 뜨는 줄인데 같은 것을 표 손잡이 메뉴가 이미 갖고 있어서, 이 판에서는 네 줄 중 둘이 상황에 따라 사라지는 모양이 됩니다. Notion 이 이 자리에 두는 열 줄 중 나머지(블록 링크 복사 · 옮기기 · 댓글 · AI)는 붙일 것이 아직 없습니다 — F4 · F5 에서 그 기능이 생길 때 같이 옵니다.

| | |
|---|---|
| 전환 | `IconArrow2ClockwiseCircularLine` — 화살표 둘이 도는 고리. 서브메뉴를 엽니다 |
| 색상 | `IconPaintrollerLine` — 기울어진 페인트 롤러 |
| 복제 | `IconSquare2StackedLine` — 겹친 사각형 둘 |
| 삭제 | `IconTrashcanLine` — 휴지통 |
| 폭 | 200px (`--knoc-block-menu-width`) — §10 Menu 기본 폭. 판이 왼쪽에 뜨는 조건이기도 합니다, §7 |
| 아이콘 크기 | 16px (§10 Menu). 슬래시 메뉴의 18px 이 아닌 것은 줄 높이가 30px · 글자가 12px 이기 때문입니다 |
| 구분선 | 없습니다 — 네 줄뿐이라 나눌 것이 없습니다 |

**아이콘은 이름이 아니라 그림으로 골랐습니다.** 넷 다 Notion 과 같은 그림입니다 — 도는 고리 ·
롤러 · 겹친 사각형 · 휴지통. seed-icon 은 이름이 뜻이 아니라 모양이라(§8), 별칭으로 먼저 좁히고
패키지 `.d.ts` 안의 `@preview` 그림을 꺼내 Notion 과 나란히 놓고 정했습니다.

전환은 `IconArrowRightArrowLeftLine`(→ ←, 별칭에 `전환`)에서 옮겼습니다. 별칭은 그쪽이 정확하지만
그림이 다릅니다 — Notion 의 전환은 **되돌아오는 고리**고 저건 **자리를 맞바꾸는 화살표**입니다.
색상도 `IconPaletteLine`(팔레트)에서 롤러로 옮겼습니다. 팔레트는 색을 **고르는 판**이고 Notion 이
그린 것은 색을 **칠하는 도구**입니다.

**네 줄 다 우리가 쓴 말입니다** (`messages.ts` 의 `blockMenuLabels`). 처음에는 전환·복제만 우리 것으로 두고 색깔·삭제는 BlockNote `ko` 사전(`drag_handle`)을 그대로 썼습니다 — 사전에 있는 말을 덮지 않으려던 것입니다. **사전의 `색깔` 을 `색상` 으로 고치면서 그 규칙이 반이 됐습니다.** 한 줄만 우리가 쓰고 옆줄은 사전이 쓰면 말투가 어긋났을 때 어디를 고쳐야 하는지가 흐려져서, 넷을 다 가져왔습니다. `삭제` 는 사전과 같은 말이지만 출처가 우리 쪽이라는 것이 중요합니다.

**`색` 이 아니라 `색상` 입니다.** Notion 은 `색` 한 글자인데, 우리 화면에서 색을 고르는 자리는 여기뿐이 아닙니다(문서 아이콘 · DB 태그가 뒤에 옵니다). 그쪽과 같은 말이어야 하고, 한 글자짜리 `색` 은 버튼 이름으로 서면 약합니다.

**삭제를 빨갛게 두지 않습니다.** §10 Menu 는 파괴적 항목을 `fg-critical` 로 하고 구분선 아래에 두라고 하지만, 그건 우리 `Menu` 컴포넌트의 규격입니다. 이 판은 BlockNote 표면이고(§7), 색을 바꾸려면 브리지에 예외를 하나 더 여는 일이 됩니다. 블록 하나를 지우는 것은 되돌릴 수 있고(⌘Z), Notion 도 이 자리를 빨갛게 두지 않습니다.

#### 전환 서브메뉴

**슬래시 메뉴와 같은 목록이 아닙니다.** 저쪽은 블록을 **넣는** 곳이라 표 · 이미지 · 목차까지 있고, 이쪽은 있는 블록을 **바꾸는** 곳입니다. 글자를 담는 블록끼리만 오갈 수 있어 열두 줄입니다.

| 묶음 | 줄 |
|---|---|
| 제목 | 제목1 · 제목2 · 제목3 |
| 기본 블록 | 본문 · 인용 · 글머리 기호 목록 · 번호 매기기 목록 · 체크리스트 · 접을 수 있는 목록 · **접을 수 있는 제목1 · 2 · 3** |

**접을 수 있는 제목 셋은 슬래시 메뉴에 없고 여기에만 있습니다.** BlockNote 가 슬래시 메뉴를 만들 때 안 넣어서 `/` 로는 못 만듭니다. Notion 도 같습니다 — 「토글 제목」은 전환 목록에만 있고, 제목을 먼저 쓰고 접을 수 있게 바꾸는 순서입니다.

**접을 수 있는 제목 셋은 맨 아래입니다.** `접을 수 있는 목록` 바로 뒤라 접히는 것 넷이 목록 끝에 모입니다 — Notion 도 토글 제목을 전환 목록 끝에 둡니다.

**이 셋만 사전 그룹을 안 따릅니다.** 이유가 둘입니다. 하나는 `ko` 사전이 이 셋을 `소제목` 에 넣는다는 것입니다 — 그 이름은 제목4·5·6 을 담으려고 지은 것이고 우리 문서에는 그 셋이 없어서(스키마가 1·2·3), 남는 것이 접을 수 있는 제목뿐인데 그걸 `소제목` 이라 부르면 다른 블록처럼 읽힙니다. 다른 하나는 머리말이 **묶음이 바뀌는 줄에만** 찍힌다는 것입니다 — 맨 아래에서 `제목` 으로 되돌아가면 한 목록에 `제목` 머리말이 두 번 섭니다. 앞 줄과 같은 묶음(`기본 블록`)으로 두면 머리말 없이 이어집니다.

**「페이지」는 아직 없습니다 — 백로그입니다** ([mvp.md 빼는 것](docs/roadmap/mvp.md)). Notion 의 전환 목록에는 블록을 하위 페이지로 바꾸는 줄이 있습니다. 한 줄 더하는 일이 아니라서 미뤄 둡니다 — 페이지를 가리키는 블록이 없고(BlockNote 에 없어서 목차처럼 우리가 만들어야 합니다), 그 자리에서 하위 페이지를 만들어야 하고(`shared/api` 를 부르는 유일한 전환입니다), **글자가 문서 밖으로 나가는 유일한 전환이라** 되돌리기가 만들어진 페이지까지 되돌려야 하는지를 같이 정해야 합니다. 나머지 열둘은 블록 하나를 제자리에서 바꾸는 일이라 이 목록 안에서 닫힙니다.

**아이콘이 제목1·2·3 과 같습니다.** BlockNote 가 둘에 같은 그림(`H1`)을 주고, 우리도 그대로 씁니다. 이름으로 갈립니다 — Notion 은 앞에 작은 꺾쇠를 붙이지만 그건 §6 이 닫혀야 손댈 자리입니다.

**목록에 없는 블록 위에서는 전환 줄 자체가 뜨지 않습니다.** 이미지 · 표 · 구분선 · 목차 · 코드 블록입니다. 종류를 갈아 끼우면 그 블록이 들고 있던 것이 갈 데가 없어서, 그건 전환이 아니라 삭제입니다. BlockNote 의 블록 타입 선택(포맷 툴바)도 같은 자리에서 같은 판단을 합니다.

이름과 묶음은 전부 `ko` 사전에서 옵니다 — 슬래시 메뉴와 같은 줄입니다. **본문이 기본 블록의 첫 줄인 것만 슬래시 메뉴와 다릅니다.** 전환은 되돌아오는 자리라 본문이 제일 많이 눌리고, 슬래시 메뉴에서 본문이 뒤쪽인 것은 빈 줄을 본문으로 넣을 일이 없어서입니다.

**아이콘은 BlockNote 기본입니다 — 슬래시 메뉴와 같은 이유입니다.** §6 "슬래시 메뉴 아이콘" 이 열려 있고, 680개 안에 제목1·2·3 을 구별할 그림이 없기 때문입니다(`IconHashLine` 하나뿐). 그 결정을 이 서브메뉴 하나 때문에 먼저 닫지 않습니다. 그동안은 슬래시 메뉴와 **같은 그림**을 씁니다 — 같은 블록을 가리키는 두 메뉴에 다른 그림이 서면 사용자가 그 둘을 다른 것으로 읽습니다. §6 이 닫히면 두 목록이 같이 갑니다.

지금 그것인 줄에는 체크가 섭니다. 코드 블록 언어 목록과 같은 이유입니다 — 열두 줄 중 어느 것이 지금인지 눈으로 알 방법이 그것뿐이고, 그림이 같은 줄(제목1 · 접을 수 있는 제목1)이 있어 더 그렇습니다.

#### 서브메뉴는 한 번에 하나만 떠 있습니다

BlockNote 는 서브메뉴가 닫혀도 판을 바로 떼지 않고 흐려지는 동안 남겨 둡니다 — `transitionProps`
에 `duration: 250` · `exitDelay: 250` 이 박혀 있어 0.5초입니다(mantine 기본은 0). 서브메뉴가
색상 하나뿐일 때는 안 보이던 일인데, 전환이 생겨 둘이 되자 **닫히는 판과 열리는 판이 같은 자리에
겹쳐 떴습니다.**

여는 쪽 상태는 mantine 이 이미 제대로 다룹니다(형제가 열리면 `registerOpenSub` 이 이쪽을 닫습니다).
남는 것은 그림뿐이라 그림을 안 그리는 것으로 끊습니다.

- **전환** — 닫혀 있으면 판을 아예 안 답니다. 언마운트라 사라지는 데 시간이 안 걸립니다
- **색상** — BlockNote 부품이라 언마운트로 못 끊습니다. 전환이 열려 있는 동안 그 판을
  (`.bn-color-picker-dropdown`) `display: none` 으로 감춥니다
