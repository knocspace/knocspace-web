...
type PageTreeStoryArgs = PageTreeProps & {
  tree: keyof typeof TREES;
  rowActions: boolean;
  loading: boolean;
};

const meta: Meta<PageTreeStoryArgs> = {
  title: "레이아웃/PageTree",
  component: PageTree,
  args: {
    items: [],
...
export default meta;
type Story = StoryObj<PageTreeStoryArgs>;
...
export const Playground: Story = {
  render: function PlaygroundStory({ tree, rowActions, loading, ...args }) {
    const [, updateArgs] = useArgs<PageTreeStoryArgs>();
    const [expanded, setExpanded] = useState<string[]>(처음펼친것);

    const 원본 = TREES[tree];
    const items = visibleItems(원본, expanded);
