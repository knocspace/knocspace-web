 ...
 *
 * - **화면에서 켜고 끄는 네 가지가 지금 다 꺼져 있습니다.** 헤더로 바꾸기 · 셀 병합 ·
 *   셀 배경색 · 셀 글자색은 에디터 옵션(`tables`)이고 넷 다 기본값이 `false` 입니다.
 *   `useEditorDoc` 에서 켜기 전까지, 표 안 메뉴에 그 항목들이 안 나옵니다
 * - 문서에 이미 담겨 있는 값은 옵션과 상관없이 **그려집니다.** 아래 컨트롤이 그걸
 *   보여줍니다 — 헤더도 병합도 색도 화면에는 나오지만, 사용자가 직접 만들지는 못합니다
...
/**
 * ### 해 볼 것
 * - **헤더 행** · **헤더 열** 을 켜 봅니다. 머리글 칸만 배경이 깔립니다
 * - **셀 병합** 을 켜 봅니다. 둘째 줄이 두 칸에서 한 칸으로 붙습니다
 * - 표 안에 커서를 두고 왼쪽 · 위쪽에 뜨는 손잡이를 눌러 봅니다 — **행 · 열 추가와 삭제만**
 *   있고 헤더 · 병합 · 색 항목은 없습니다. 그게 옵션이 꺼져 있다는 뜻입니다
