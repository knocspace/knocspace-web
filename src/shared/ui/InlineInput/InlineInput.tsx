...
export interface InlineInputProps {
   ...
   * bare   큰 글자 자리. 테두리도 배경도 없이 캐럿만
   *
   * 큰 글자는 그 자체로 자기 영역이 뚜렷해서, 상자를 더하면 화면이
   * 양식처럼 보인다. 대신 평상시 호버 배경도 없어서 "여기를 눌러 고친다"
   * 는 신호가 약해진다.
   *
   * **문서 제목은 이걸 안 쓴다.** 40px 제목은 잘리지 않고 줄바꿈해야 하는데
   * 이 컴포넌트는 두 상태 모두 truncate 다 — components/PageTitle 참고.
   */
  variant?: "boxed" | "bare";
