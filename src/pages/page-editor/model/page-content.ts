 ...
 */
export function toBlockNoteInitialContent(content: PageContent | undefined): KnocPartialBlock[] | undefined {
  if (!content || content.blocks.length === 0) return undefined;
  return content.blocks as KnocPartialBlock[];
}

/** 에디터가 들고 있는 것을 저장할 모양으로 담는다. */
export function toPageContent(blocks: readonly unknown[]): PageContent {
