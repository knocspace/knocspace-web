 ...
 *
 * | | BlockNote | 더 가진 props |
 * | --- | --- | --- |
 * | 이미지 | `image` | `showPreview` `previewWidth` `textAlignment` |
 * | 동영상 | `video` | 이미지와 같음 |
 * | 오디오 | `audio` | `showPreview` (폭 없음) |
 * | 파일 | `file` | 없음 |
 * | PDF | **없음** | 파일 블록으로 대신합니다 |
 *
 * - **업로드 핸들러(`uploadFile`)가 아직 안 붙어 있습니다.** 지금은 URL 로만 넣을 수 있고,
 *   파일 서버가 생기는 F4 이후의 일입니다
 * - `url` 이 비면 **"파일 추가" 자리** 로 뜹니다. 블록을 막 만들었을 때의 첫 모습입니다
...
type FileStoryArgs = Omit<ContentEditorProps, "pageId" | "content"> & {
  kind: FileKind;
  hasUrl: boolean;
  caption: string;
  name: string;
...
/**
 * ### 해 볼 것
 * - **URL 있음** 을 꺼 봅니다. 네 종류 모두 `파일 추가` 자리로 바뀝니다
 * - **미리보기 폭** 을 바꾸거나 이미지 모서리를 끌어 봅니다. 그 값이 `previewWidth` 로 저장됩니다
 * - **미리보기** 를 끄면 이미지가 파일 한 줄로 접힙니다
 * - 캡션을 지워 봅니다. 줄 자체가 없어져서 아래 본문이 붙습니다
 *
 * 동영상 · 오디오는 견본 URL 이 가짜라 재생되지 않습니다 — 여기서 볼 것은 블록의 자리와 여백입니다.
 */
