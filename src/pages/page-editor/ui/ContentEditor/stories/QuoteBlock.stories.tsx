 ...
 * | --- | --- | --- |
 * | 블록 | `quote` | `quote` |
 * | 입력 | `> ` | `" ` (따옴표 뒤 공백) — `> ` 는 토글로 넘겼습니다 |
 * | 단축키 | — | `Ctrl+Alt+Q` |
 *
 * - **막대와 글자색은 브릿지가 SEED 로 넘긴 값입니다.** BlockNote 가 `#7d797a` 를 CSS 에
 *   박아 둬서 변수로는 안 닿고, 자손 선택자 두 줄로 `fg-neutral-muted` 를 덮습니다 (DESIGN.md §7)
 * - **막대 굵기는 안 건드립니다.** 색만 덮어서, BlockNote 가 2px 을 3px 로 바꾸면 그대로 따라옵니다
 * - BlockNote 문서에는 색 props 만 적혀 있지만, 자식 블록도 받습니다
 */

type QuoteStoryArgs = Omit<ContentEditorProps, "pageId" | "content"> & {
  text: string;
...
/**
 * ### 해 볼 것
 * - **글자색** 을 기본에서 회색으로 바꿔 봅니다. 브릿지가 덮은 색과 팔레트의 회색이 다릅니다
 * - 다크로 뒤집어 봅니다. 막대도 글자도 같이 뒤집혀야 합니다
 * - 빈 줄에서 `" ` 를 쳐 봅니다. 인용이 됩니다 (`> ` 는 토글입니다)
