 ...
 * | --- | --- | --- | --- |
 * | 굵게 · 기울임 · 밑줄 · 취소선 · 코드 | `annotations` 5종 | 같은 5종 | `**` `*` `~` `` ` `` |
 * | 글자색 · 배경색 | 고정 9색 enum | 자유 문자열 | — |
 * | 링크 | `text.link` | `link` | `[글자](주소)` |
 * | 이모지 | `:이름:` | `:` 로 이모지 메뉴 | — |
 * | 멘션 · 인라인 수식 | 있음 | **없음** | — |
 *
 * - **다섯 서식은 완전히 같습니다.** 이름도 마크다운 표기도 양쪽이 같아서, Notion 에서
 *   복사해 붙여도 그대로 옵니다
 * - **멘션은 직접 만들어야 합니다.** BlockNote 기본 인라인 스펙은 글자와 링크 둘뿐이라
 *   `@사람` · `@페이지` 는 `createReactInlineContentSpec` 쪽 일입니다 (`knocspace-parity.md` — P1)
 * - 배경색은 형광펜입니다. SEED 팔레트가 모드에 따라 뒤집혀 라이트 · 다크가 한 벌로 맞고,
 *   **갈색 · 분홍만 팔레트 밖** 이라 결이 다릅니다 (DESIGN.md §7)
 */
...
type InlineStoryArgs = Omit<ContentEditorProps, "pageId" | "content"> & {
  styles: InlineStyle[];
